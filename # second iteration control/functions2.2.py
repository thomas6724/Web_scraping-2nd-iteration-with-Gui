...
def input_code():
    ...
    # accepts input
        ...
        numError = Label(main_window, text="Please do not enter numbers")
        numError.grid(row=2)
    ...
    try:
        if len(user_input.strip()) <= 1 or len(user_input.strip()) >= 30:
            ...
            errormessage1 = Label(main_window, text="Please enter a valid message")
            errormessage1.grid(row=2)
    except ValueError:
        pass
...
def web_scraper():
    ...
    for container in containers:
        ...
        file.write(str(brand) + "," + str(model.replace(",", "|")) + "," + str(price) + "$USD," + str(origin_price) +
...
Label(main_window, text="Type in something to search").grid(row=0)
input_test = Entry(main_window, width=50, borderwidth=5)
input_test.grid(row=1, column=0)
