        ...
        try:
            ...
            for container in containers:
                ...
                try:
                    ...
                    # if there isn't a word in neweggs html file, specifically title under the dictated directory -
                    # skips rating if there isn't an index that is in reach or rating on a product, writing 'none' in
                    # the ratings place writing 'none' in the ratings place
                    rating = "none"
                ...
                try:
                    ...
                    price = Gui().remove_letters(price)
                    price = re.sub('[^0-9.]', '', price)

                except IndexError:

                    # if there isn't an index that is in reach or price on a product, writing 'none' in the price's
                    # place
                    price = "none"
                ...
                # writing the results of the web-scraper to the CSV file, as it loops around until it reaches the
                # bottom of the list/ last container on the page
                file.write(
                    str(brand) + "," + str(model.replace(",", "|")) + "," + str(price) + "," + str(origin_price) +
            ...
            # showing label
            selection_label = Label(main_window, text="Please select display option and click save selection")
            selection_label.grid(row=2, column=0)
...
class Gui:
    ...
    def introduction():
        ...
        intro = Label(main_window, text="Hello, welcome to the Newegg.com web-scraper! Using this web-scraper you can\n"
                                        "search up your favourite products, getting real time data. For your "
                                        "convenience\n "
                                        ...
                                        "category's include but are not limited to - Graphics cards, Ram, PC cases "
                                        "and\n "
    ...
    def input_code():
        ...
        # covers up error message
        cover_label = Label(main_window, text='|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||'
                                              '|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n'
                                              '|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||'
                                              '|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n'
                                              '|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||'
        ...
        while True:
            ...
            # accepts input
                ...
                errormessage1 = Label(main_window, text="Please do not enter numbers!", fg='red')
            ...
            # does not accept more than 4 words
                ...
                errormessage1 = Label(main_window, text="Please type no more than 4 words", fg='red')
                errormessage1.grid(row=1, column=0)
                break
            ...
            # picks up all typos/ non english words and symbols
                ...
                errormessage1 = Label(main_window,
                                      text="Please enter a word in the english dictionary. \n"
                                           "The word '" + user_input + "' is not valid",
                                      fg='red')
                errormessage1.grid(row=1, column=0)
                break
    ...
    def run():
        ...
        """
        filename = Webscraper().web_scraper()
    ...
    def gui():
        ...
        searchbutton = Button(main_window, text="Search", command=Gui().search)
...
drop = OptionMenu(main_window, clicked, "Select", "Show all")

# Shows data
save_selection = Button(main_window, text="Save selection", command=Gui().work)
